...
class ReviewDocumentRetriever:
    ...
    def retrieve(self,query,top_k):
        ...
        return [RetrievedDocument(f"review:{h.document_id}",str(h.metadata.get("content") or h.title),float(score),"review",{**h.metadata,"title":h.title,"source_local_score":h.score}) for h,score in ranked[:top_k]]
